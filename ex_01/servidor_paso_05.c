/*
 * gcc -Wall servidor_paso_05.c sock-lib.c -o servidor -lpthread
 *
 *  guardo la informacion recibida en el array de estructuras(global)
 *  agrego la funcionalidad de servidor multithread
 *  agrego la funcionalidad de vaciar el array con el thread dedicado 'monitor'
 *
 */
...
// funcion que ejecutara el thread 'monitor'
...
void *monitor(void *ptr)
{
    while (1)
    {
        ...
        if (idx == STATUS_SIZE)
        {
            dump();  // imprimo por pantalla el array
            idx = 0; // vacio el array
        }
        else
        {
            if (debug)
                printf("debug:: monitor() tamaño del array...[len=%d]\n", idx);
        }
        pthread_mutex_unlock(&mutex);
        // usleep(5000);
        sleep(1);
